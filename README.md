...
| 代號 | 名稱 | 嚴重度 | 說明 |
|------|------|--------|------|
| P0 | 硬性停損 | 4 | 虧損超過可承受範圍。單獨觸發就足以要求全數出場 |
...
**停損距離隨波動縮放**（`core/risk.py`，設定在 `config/rules.yaml` 的
頂層 `stop:`）：3 個日標準差，夾在 -4% ~ -8% 之間。固定百分比對年化波動
0.15 的標的等於 9 個標準差、形同沒有停損，對 0.6 的標的又只有 2 個標準差、
正常盤整就被洗掉。

進場端算出的停損價與出場端 P0 的觸發線**是同一個數字** —— 兩邊共用
`core/risk.py`。修正前進場端會說「買 156、停損 139」，而出場端從來不檢查。

## 已驗證行為

`pytest tests/ -q` → 179 passed。以下每一條都有對應測試：
